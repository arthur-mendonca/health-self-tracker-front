...
<div class="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
  ...
  {#if store.error}
    ...
    >
      ...
      <button
        ...
        onclick={() => (store.error = null)}>X</button
      >
    </div>
  {/if}

  <div
    class="flex items-center space-x-1 rounded-md bg-muted p-1 sm:max-w-md w-full"
    role="tablist"
  ...
  >
    <button
      ...
      class="flex-1 rounded-sm px-3 py-1.5 text-sm font-medium transition-all {store.activeTab ===
      'tags'
  ...
  <div class="relative min-h-[400px]">
      ...
      >
        <ManageTags
          ...
          onCreate={async (name, category) => {
            await store.createNewTag(name, category);
          }}
    ...
    {:else if store.activeTab === "substances"}
      <section
        aria-label="Gerenciar Substâncias"
        class="fade-in animate-in duration-200"
      >
        <ManageSubstances
          items={store.substances}
          isLoading={store.isSubstancesLoading}
          onCreate={async (name, type, dose) => {
            await store.createNewSubstance(name, type, dose);
          }}
    ...
    {:else if store.activeTab === "activities"}
      ...
      >
        <ManageActivities
          ...
          onCreate={async (name) => {
            await store.createNewActivity(name);
          }}
    ...
    {#if store.isTagsLoading && store.isSubstancesLoading && store.isActivitiesLoading && !store.tags.length && !store.substances.length && !store.activities.length}
      <div
        class="absolute inset-0 z-10 flex items-center justify-center rounded-xl bg-background/50 backdrop-blur-[1px]"
      >
